...
<div class="space-y-4">
    <div class="flex justify-between items-center">
        ...
        <a href="#" class="text-[10px] font-bold text-[#f2994a] uppercase tracking-widest hover:underline flex items-center gap-1">
    ...
    <div class="relative group">
        ...
        <div class="overflow-hidden rounded-2xl">
            <div class="packages-carousel-wrapper flex gap-6 pb-2 transition-transform duration-300 ease-out"
                 style="scroll-behavior: smooth;">

                @forelse($packages as $package)
                <div class="packages-carousel-item flex-shrink-0 w-80" data-package-id="{{ $package->id_layanan }}">
                    <!-- Card Package -->
                    <div class="h-full bg-gradient-to-br from-white/[0.08] to-white/[0.02] border border-white/10 rounded-2xl overflow-hidden group/card hover:border-[#f2994a]/50 transition-all duration-300 shadow-lg hover:shadow-xl">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const initCarousel = () => {
            ...
            document.querySelectorAll('.packages-carousel-wrapper').forEach(wrapper => {
                const parentGroup = wrapper.closest('.group');
            ...
            try {
                const response = await fetch('/api/keranjang/item', {
                    ...
                    headers: {
                        ...
                        'Authorization': 'Bearer ' + (document.querySelector('meta[name="api-token"]')?.content || localStorage.getItem('api_token'))
                    },
                    body: JSON.stringify({
        ...
        const orderNow = (packageId, packageName) => {
            // Redirect ke halaman checkout/pesanan dengan package ID
