            ...
            <div class="lg:col-span-8 space-y-8" data-aos="fade-right" data-aos-duration="1200">
                ...
                <div class="space-y-4">
                    <h1 class="text-4xl sm:text-5xl lg:text-6.5xl font-extrabold text-white leading-[1.1] tracking-tight">
                        {{ \App\Helpers\StaticContent::HERO_TITLE_1 }} <br>
                        <span class="text-gradient">{{ \App\Helpers\StaticContent::HERO_TITLE_2 }}</span>
                    </h1>
                    <p class="text-gray-300 text-sm sm:text-base md:text-lg leading-relaxed max-w-xl">
                        {{ \App\Helpers\StaticContent::HERO_SUBTITLE }}
                    </p>
                </div>
                <!-- CTAs -->
                <div class="flex flex-wrap items-center gap-4 sm:gap-6 pt-2">
                    <a href="{{ auth()->check() ? route('katalog.user') : route('layanan') }}" class="btn-premium px-8 py-4 rounded-xl font-bold text-xs sm:text-sm uppercase tracking-widest text-black flex items-center gap-3 transition-all hover:scale-105 active:scale-95 shadow-[0_4px_20px_rgba(242,153,74,0.3)]">
                        {{ \App\Helpers\StaticContent::CTA_PESAN_SEKARANG }} <i class="ph-bold ph-arrow-right text-base"></i>
                    </a>
                    <a href="#mahakarya" class="px-8 py-4 rounded-xl border border-white/20 text-white font-bold text-xs sm:text-sm uppercase tracking-widest bg-white/5 hover:bg-white/10 hover:border-white/40 transition-all">
                        {{ \App\Helpers\StaticContent::CTA_LIHAT_SEMUA }}
                    </a>
                </div>
                <!-- Micro Stats -->
                <div class="flex items-center gap-12 pt-8 border-t border-white/5 max-w-md">
                    <div>
                        <h3 class="text-3xl font-extrabold text-white">{{ \App\Helpers\StaticContent::HERO_STAT1_VAL }}</h3>
                        <p class="text-gray-400 text-xs mt-1 uppercase tracking-wider">{{ \App\Helpers\StaticContent::HERO_STAT1_LBL }}</p>
                    </div>
                    <div class="w-px h-8 bg-white/10"></div>
                    <div>
                        <h3 class="text-3xl font-extrabold text-[#f2994a]">{{ \App\Helpers\StaticContent::HERO_STAT2_VAL }}</h3>
                        <p class="text-gray-400 text-xs mt-1 uppercase tracking-wider">{{ \App\Helpers\StaticContent::HERO_STAT2_LBL }}</p>
                    </div>
                </div>
            </div>
        </div>
